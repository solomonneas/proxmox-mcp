...
export function createProxmoxListVmsTool(getClient: ClientFactory) {
  return {
    ...
    description:
      "List every QEMU VM across the cluster (vmid, name, node, status, CPU/mem) via GET /cluster/resources?type=qemu.",
    parameters: Schema,
    execute: async () => {
      const client = getClient();
      const vms = await client.get<VmResource[]>("/cluster/resources?type=qemu");
